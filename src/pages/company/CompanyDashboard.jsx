...
import { Briefcase, Users, Plus, Calendar, MapPin, DollarSign, Clock, CheckCircle, XCircle, TrendingUp, AlertCircle, FileText, Activity } from "lucide-react";
...
export default function CompanyDashboard() {
    ...
    // Calculate Stats
    // Use backend quota if available, otherwise fallback to rough frontend calc
    const placesLeft = profile?.quota?.remaining ??
        Math.max(0, jobs.reduce((acc, job) => acc + (job.interviewQuota || 10), 0) - jobs.reduce((acc, job) => acc + (job.applicationCount || 0), 0));
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                ...
                {/* Dynamically added 'Places Disponibles' logic handled in stats array definition below */}
            </div>
                                ...
                                <div className="pt-5 border-t border-white/10 mt-auto flex items-center justify-between gap-4">
                                    <div className="flex flex-col gap-1 w-full max-w-[50%]">
                                        <div className="flex justify-between items-center text-xs font-bold text-slate-400">
                                            ...
                                            <span className={`${Math.max(0, (job.interviewQuota || 10) - (job.acceptedCount || 0)) <= 2 ? 'text-red-400 animate-pulse' : 'text-emerald-400'}`}>
                                                {Math.max(0, (job.interviewQuota || 10) - (job.acceptedCount || 0))}
                                            </span>
                                        </div>

                                        {(job.acceptedCount || 0) >= (job.interviewQuota || 10) ? (
                                            ...
                                            <div className="w-full bg-slate-800 rounded-full h-1.5 overflow-hidden">
                                                <div
                                                    className={`h-full rounded-full ${Math.max(0, (job.interviewQuota || 10) - (job.acceptedCount || 0)) <= 2 ? 'bg-red-500' : 'bg-blue-500'}`}
                                                    style={{ width: `${Math.min(100, ((job.acceptedCount || 0) / (job.interviewQuota || 10)) * 100)}%` }}
                                                />
                                            </div>
                                        )}

                                        <div className="text-[10px] text-slate-500 mt-0.5 flex justify-between">
                                            <span>{job.acceptedCount || 0} / {job.interviewQuota || 10} Confirmés</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            ...
                                            className="w-10 h-10 rounded-xl bg-slate-800/80 hover:bg-white text-slate-300 hover:text-slate-900 transition-all border border-white/10 hover:border-white shadow-lg flex items-center justify-center group/edit"
                                            title="Modifier"
                                        >
                                            <Briefcase size={18} className="group-hover/edit:scale-110 transition-transform" />
                                        </button>
                                        <button
                                            onClick={async () => {
                                            ...
                                            title="Supprimer"
                                        >
                                            <XCircle size={18} className="group-hover/del:rotate-90 transition-transform" />
                                        </button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 sm:p-6 w-full max-w-lg shadow-2xl animate-fade-in-up">
                        ...
                        <form onSubmit={handleCreateJob} className="space-y-4 sm:space-y-5">
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
                                ...
                                <button
                                    type="submit"
                                    className="px-8 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-blue-600/20 hover:scale-105"
                                >
                                    {newJob.id ? "Enregistrer" : "Publier l'offre"}
                                </button>
                            </div>
                        </form>
                    </div>
