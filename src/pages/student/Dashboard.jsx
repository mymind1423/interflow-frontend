...
import { Briefcase, Calendar, Search, Building, TrendingUp, Bookmark, ChevronRight, QrCode, Trash2, Info, Clock, CheckCircle2, RotateCcw, Sparkles } from "lucide-react";
...
function Dashboard() {
  ...
  const [stats, setStats] = useState({ applications: 0, interviews: 0, saved: 0, views: 0, completion: 0 });
  const [profile, setProfile] = useState(null);
  const [recentJobs, setRecentJobs] = useState([]);
  const [applications, setApplications] = useState([]);
  const [nextInterview, setNextInterview] = useState(null); // New state for next interview
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [statsData, profileData, jobsData, appsData, interviewsData] = await Promise.all([
          studentApi.getStats(),
        ...
        if (interviewsData && interviewsData.length > 0) {
          ...
          if (sorted.length > 0) {
            ...
            setNextInterview({
              ...
              time: next.dateObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
              link: next.meetLink,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-8 mt-6 pb-20 relative">
      ...
      >
        <div>
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold text-white mb-2 leading-tight">
            Bonjour, <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">{displayName}</span>
          </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <StatCard label="Entretiens" value={stats.interviews || 0} color="text-violet-400" bg="bg-violet-400/10" icon={Calendar} delay={0.2} />
        <StatCard label="Sauvegardées" value={stats.saved || 0} color="text-pink-400" bg="bg-pink-400/10" icon={Bookmark} delay={0.3} />
        <StatCard label="Vue Profil" value={stats.processedApplications || 0} color="text-emerald-400" bg="bg-emerald-400/10" icon={TrendingUp} delay={0.4} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="w-full overflow-x-auto no-scrollbar pb-2">
            <div className="bg-slate-900/50 p-1.5 rounded-2xl inline-flex border border-white/5 min-w-fit">
              {['overview', 'jobs', 'applications'].map(tab => (
                ...
                >
                  {tab === 'overview' ? 'Aperçu' : tab === 'jobs' ? 'Offres' : 'Candidatures'}
                </button>
              ))}
            </div>
          </div>
          ...
          {(activeTab === "overview" || activeTab === "applications") && (
            <motion.div layout className="mt-8">
              ...
              <div className="space-y-4">
                  ...
                  <div key={app.id} className="group bg-slate-900/40 backdrop-blur-sm border border-white/5 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between hover:bg-white/5 hover:border-white/10 transition-all cursor-pointer gap-4 sm:gap-0">
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-1">
                          <span className="text-xs text-slate-500 bg-slate-800/50 px-2 py-0.5 rounded border border-white/5 whitespace-nowrap">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                      <span className={`px-3 sm:px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide border whitespace-nowrap ${app.status === 'ACCEPTED' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
                        ...
                        {app.status === 'PENDING' ? 'Envoyé' :
                          app.status === 'ACCEPTED' ? 'Invité' :
                            app.status === 'INVITED' ? 'Invité' : app.status}
                      </span>
                      {app.status === 'PENDING' && (
                        <button
                          onClick={(e) => { e.stopPropagation(); /* Add delete logic if needed here or in parent */ }}
                ...
                {applications.length === 0 && <p className="text-slate-500 text-center py-8">Vous n'avez pas encore candidaté.</p>}
              </div>
            </motion.div>
          )}

          {/* Token Info Widget (Moved to bottom of Left Column) */}
          <TokenInfoWidget tokens={profile?.tokensRemaining} />
        ...
        >
          {/* 1. Interview Widget */}
          <InterviewWidget nextInterview={nextInterview} />
